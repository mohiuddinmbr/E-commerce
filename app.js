...
let render = (arr) => {
    ...
    <h1>${item.name}</h1>
     ...
     <p>${item.category}</p>
     <p>${item.stock}</p>
     <button class="cart-button" onclick="addcart(${item.id})">Add to cart</button>
    </div>
...
let addcart = (id) => {
    const single = products.find(item => {

        return item.id === id
    })
    console.log(single);

    if (cartItems.includes(single)) {
        console.log('item already exist');

        let index = cartItems.findIndex(item => item.id === single.id)
        console.log(index);

        cartItems[index].quentity += 1

    } else {
        single.quentity = 1
        cartItems.push(single)
    }
}
